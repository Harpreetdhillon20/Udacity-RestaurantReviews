  ...
  fetchRestaurantFromURL((error, restaurant) => {
    if (error) { // Got an error!
      ...
      L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.jpg70?access_token={mapboxToken}', {
        mapboxToken: 'sk.eyJ1IjoiaGFycnlkaGlsbG9uIiwiYSI6ImNqcTNqZWo5cjFmNzg0OG11NHhsdzc4YjcifQ.Ica-HI5WCb3hN3R2YxfAfQ',
        maxZoom: 18,
...
fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  image.alt = `Snapshot of ${restaurant.name} restaurant`;

  const cuisine = document.getElementById('restaurant-cuisine');
  cuisine.innerHTML = restaurant.cuisine_type;
  cuisine.setAttribute('aria-label', `Cuisine served ${restaurant.cuisine_type}`);
